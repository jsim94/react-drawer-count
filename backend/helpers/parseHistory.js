...
function parseHistory({ currencyCode, drawerAmount, denominations }) {
  ...
  if (currencyJson.currencies[currencyCode] === undefined)
    throw new Error(`currencyCode '${currencyCode}' invalid.`);
  if (drawerAmount < 0)
    throw new Error(`drawerAmount '${drawerAmount}' must be greater than 0`);

  const CURRENCY = currencyJson.currencies[currencyCode];

  if (denominations.length !== CURRENCY.bills.length + CURRENCY.coins.length)
    throw new Error(`denominations length does not equal currency length`);

  const values = [];
  CURRENCY.bills.forEach((bill) => values.push(bill));
  CURRENCY.coins.forEach((coin) => values.push(coin));

  let inputTotals = denominations.reduce(
    (acc, qty, idx) => acc + qty * values[idx].value * 100,
    0
  );

  const depositTotal = inputTotals - drawerAmount * 100;
  let total = 0;

  const depositQtys = denominations.map((qty, idx) => {
    const currValue = values[idx].value * 100;
