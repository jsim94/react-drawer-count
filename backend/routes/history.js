...
router.post("/submit", ensureLoggedIn, async (req, res, next) => {
  try {
...
/** DELETE /user/:username/delete-history
 *
 *  Authorization required: same user
 */

router.delete(
  "/user/:username/delete-history",
  ensureLoggedIn,
  async (req, res, next) => {
    try {
      const username = req.params.username;
      if (username !== res.locals.user.username) throw new UnauthorizedError();
